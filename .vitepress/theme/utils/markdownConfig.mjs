...
const icons = {
  ...
  warning:
    "<svg viewBox=\"0 0 24 24\" width=\"16\" height=\"16\" fill=\"currentColor\"><path d=\"M12 2L1 21h22L12 2zm0 4l7.5 13h-15L12 6zm-1 5v4h2v-4h-2zm0 6v2h2v-2h-2z\"/></svg>",
  ...
  danger:
    "<svg viewBox=\"0 0 24 24\" width=\"16\" height=\"16\" fill=\"currentColor\"><circle cx=\"12\" cy=\"12\" r=\"10\"/><line x1=\"15\" y1=\"9\" x2=\"9\" y2=\"15\" stroke=\"white\" stroke-width=\"2\"/><line x1=\"9\" y1=\"9\" x2=\"15\" y2=\"15\" stroke=\"white\" stroke-width=\"2\"/></svg>",
...
const markdownConfig = (md, themeConfig, { posts = [], tags = [] } = {}) => {
  ...
  md.core.ruler.after("block", "obsidian-callout", (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      tokens[i].content =
        `<div class="${config.className} custom-block"><p class="custom-block-title">${icon} ${title}</p>`;
      tokens[i].tag = "";
      ...
      if (inline.children) {
        ...
        for (const child of inline.children) {
          if (skipNext && child.type === "softbreak") {
            skipNext = false;
            continue;
          }
