...
topic: [transformer, kv-cache, attention]
type: article
created: 2026-01-02
cover: https://image.phimes.top/img/20260101232311654.png
---
## 1 TL;DR
...
$$
\text{Attn}(\vec{q_t})​=Softmax(\vec{q_{t}}​K_{new}^T​)V_{new}​
$$
...
除了显存问题（不是主要问题），更麻烦的其实是**显存带宽**。GPU中有两个部分，一个是HBM（显存，有时候我们叫它VRAM）另一个是SRAM（计算单元）。我们的attention计算是在计算单元中的，但是缓存的KV是在HBM里的。
...
注意：这里不写计算的时间是因为内存墙会导致搬运时间必然大于计算时间，所以没必要再看计算的强度了。他们之间的比值可以作为浪费的参考。
