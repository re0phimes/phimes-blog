...
topic: [transformer, attention, qkv]
type: article
created: 2025-01-29
cover: https://image.phimes.top/img/20250129225821.png
---
...
这里我们可以先解释$\sqrt{d_{k}}$，其中$d_k$是Q和K向量的维度，$\sqrt{d_k}$是其平方根，作为缩放因子。为什么要缩放？如果$QK^T$的结果非常大，那么经过$softmax$之后就会出现**饱和现象**：某些值趋近1，其余趋近0。在饱和区域，softmax的梯度极小，不利于模型训练。所以除以$\sqrt{d_k}$，就是为了防止点积过大导致softmax饱和
...
而每一个token会把自己的特征表示放在K矩阵中。注意，$K_i$不是专门回答$Q_i$的——每个$Q_i$会和序列中**所有**token的$K_j$做点积，得到token $i$对每个token $j$的注意力分数。
...
$Q$和$K^T$是进行点积，在[[Transformer中的Q和K 2025-01-29#2 前置知识#2.1 dot product的现实意义]]前置知识部分，我们已经说过：
...
>
>通过$\sqrt{d_{k}}$进行缩放，防止点积过大导致softmax饱和（饱和区域梯度极小，不利于训练）
>
