...
在这部分，我会说一下embedding的的概念，通过流形（Manifold）去从理论角度理解embedding layer，再通过实际的embedding model的运用。说明现代llm中RAG的理论指导实践的具体体现。
...
![transformer架构下的embedding layer](https://image.phimes.top/img/202508291836152.webp)
...
![欧式空间下的向量语义线性类比](https://image.phimes.top/img/202508291900282.webp)
...
![多个流形](https://image.phimes.top/img/20250829232129706.webp)
...
在训练多个垂直领域（垂类）文本的llm时，模型学习到的嵌入空间对应一个**连续的低维流形**。这个流形不是孤立的碎片，而是一个连贯的整体，但其中包含多个**语义簇**，每个簇对应一个垂类的核心概念。这些簇之间通过**共享词汇、抽象语义或上下文关系**相互连接，形成平滑的过渡区域。例如，医疗领域的“操作”和科技领域的“操作”可能在不同簇中，但通过通用含义连接。这种连续性使模型能够处理跨领域任务，而簇的存在反映了领域特异性。流形的具体几何结构，取决于训练数据的多样性和模型架构。

在这个图中，我们可以把每个小山峰都理解为一个垂类领域，哪些低洼地带依然是不平整的，他们是语义稀薄的区域，但是没有断裂，而多个山丘就组成了语义簇，这个语义簇可能也就是我们语义流形的一个局部而已：

![山峰视角的流形局部](https://image.phimes.top/img/202508281908376.webp)
...
![叽里咕噜说什么呢](https://image.phimes.top/img/20250821153536.webp)
...
这些问题是不管是工作还是搞科研，还是你单纯爱学习都不可避免的。所以我们**接下来从流形视角看看llm几个常见问题。**
...
**从Embedding和流形角度就是**，基础llm通过海量数据，已经学习到了一个覆盖人类通用知识的、极其稳健的语义流形。当我们用一个垂类的高质量数据进行微调时，我们并不是要从零开始学习一个新的流形。我们的目标仅仅是对某个特定区域，比如“法律”或“医疗”领域，进行一次**平滑的、局部的“塑形”或“扭曲”**。
...
![流形中的距离计算](https://image.phimes.top/img/202508281655077.webp)
...
![metadata示意图](https://image.phimes.top/img/202508291511611.webp)
