...
第二个核心结论是：**多个线性变换的复合（矩阵乘法）等效于一个单一的线性变换，也就是$(XW_{1})W_{2}=X(W_{1}W_{2})$，最终等效于一次变换 $XW$**

让我们回顾一下FFN公式

$$FFN(x)=Activation(xW_1+b_1)W_2+b_2$$
如果我们没有$Activation$这个激活函数，那我们的$FFN$的两次线性变换就相当于上面的$(XW_{1})W_{2}=X(W_{1}W_{2})$，**这两次线性变换实际就退化成了一层**
...
> [!NOTE]
> 这只是一个简化的具象化示例，便于理解，但是不能替代专业术语说明
