...
type: article
created: 2025-01-10
cover: https://image.phimes.top/img/202501100954832.png
---
...
> 从最基本的形式来看，智能体可以被定义为一个通过观察世界并使用其可用工具采取行动来实现目标的应用程序。智能体是自主的，可以独立于人类干预而行动，特别是当被赋予适当的目标或要实现的目标时。智能体在实现目标的方法上也可以是主动的。即使在没有来自人类的明确指令集的情况下，代理也可以推理下一步应该做什么来实现其最终目标。为了理解代理的内部运作，需要了解智能体的行为、行动和决策的基础组件。这些组件的组合可以被描述为认知架构，通过这些组件的混合和匹配可以实现许多这样的架构。聚焦于核心功能，代理的认知架构中有三个基本组件[^1]

## Agent、model以及workflow
![google《agents》 models vs agents](https://image.phimes.top/img/202501100954832.webp)

google已经清晰阐述了models和agents的对比：
- 知识方面，agents可以基于工具对外部知识进行扩展
- 交互方面，models本身不具备逻辑，主要是基于用户的输入进行预测，而agents通过维护一个session histroy（chat history）来获得多次的交互。
- agents支持工具调用。
...
所以workflows具有如下特点：
- 明确定义的输入输出
- 支持工具、models、agents通过指定的程序逻辑进行执行和调用
- 具有一定的条件分支、决策控制、并行处理或者循环迭代的能力

![一种workflow的例子](https://image.phimes.top/img/202501101128193.webp)
...
![image.png](https://image.phimes.top/img/202501101129372.webp)
## model
