...
type: article
created: 2025-06-12
cover: https://image.phimes.top/img/202506101120234.png
---
## 写在前面

在我最早学习LLMs时候，残差连接这个词看着就很高大上，让我望而生畏，极大的延缓了我系统的学习的进度。但是，因为这样那样的原因，最终我还是恶补了各种知识。学完之后觉得也就那样了，所以，我希望初学者看到transformer里藏着这样一个部分的时候，可以不用过多担心，直接开始看。

![transformer结构](https://image.phimes.top/img/202506101120234.webp)
...
![简化的残差网络的例子](https://image.phimes.top/img/202506091116077.webp)
...
站在**特征融合**的视角下，在图像也就是CNN和语言类（在这里，我们特指bert或者语言任务的transformer），实际的含义是不同的。
...
![特征操作](https://image.phimes.top/img/20250611214441725.webp)
...
**过大的学习率**也会导致权重更新步长失控，可能使$F(x)$及其梯度在单次迭代中剧增，破坏数值稳定性。通常是使用**学习率预热**在初期使用小学习率稳定网络，随后提升；配合**学习率衰减**在训练后期精细调整步长。
## Norm
...
`Add` 操作（残差连接）本身会**显著改变数据的分布**。即使两个相加的变量（如输入 和前层输出 ）具有相似的分布，它们的和也会形成一个新的分布。例如，若假设两者近似独立：
